...
SIGNED_EVENTS_PAGE_SIZE = 25
SIGNED_EVENTS_MAX_PAGES_PER_RANGE = 20
SIGNED_EVENTS_MIN_RANGE = timedelta(hours=1)
...
class KedoSignedDocument(BaseModel):
    ...
    raw_signed_content: dict[str, Any] = Field(default_factory=dict)
    raw_signature_wrapper: dict[str, Any] = Field(default_factory=dict)
    raw_signature: dict[str, Any] = Field(default_factory=dict)
...
class KedoSignedDocumentsResponse(BaseModel):
    ...
    raw_signed_events: list[dict[str, Any]] = Field(default_factory=list)
    raw_processes: list[dict[str, Any]] = Field(default_factory=list)
    event_query_errors: list[dict[str, Any]] = Field(default_factory=list)
    process_errors: list[dict[str, Any]] = Field(default_factory=list)
...
def get_signed_documents(
    ...
    *,
    ...
    page_limit = min(safe_limit, SIGNED_EVENTS_PAGE_SIZE)
    ...
    with httpx.Client(base_url=_base_url(settings), timeout=120.0) as client:
        ...
        if days:
            ...
            from_time = to_time - timedelta(days=safe_days)
            query["eventTimeRange"] = {
                "from": _kedo_datetime(from_time),
                "to": _kedo_datetime(to_time),
            }
            ranges = _kedo_event_time_ranges(from_time, to_time)
        else:
            ranges = [(None, None)]

        events, last_offset, event_query_errors = _query_kedo_process_events(
            client,
            settings,
            token,
            api_key,
        ...
        signed_events = [
            event for event in events if _optional_int(event.get("eventType")) == 10
        ]
    ...
    return KedoSignedDocumentsResponse(
        ...
        query=query,
        raw_events=events,
        raw_signed_events=signed_events,
        raw_processes=processes,
        event_query_errors=event_query_errors,
        process_errors=process_errors,
        signed_documents=signed_documents,
    )


def _query_kedo_process_events(
    client: httpx.Client,
    settings: Settings,
    access_token: str,
    api_key: str,
    ...
    *,
    ...
    for index, (from_time, to_time) in enumerate(ranges):
        range_events, range_last_offset = _query_kedo_process_events_range(
            client,
            settings,
            access_token,
            api_key,
            ...
            to_time=to_time,
            errors=errors,
            offset=offset if index == 0 else None,
        )
        events.extend(range_events)
        if range_last_offset:
            last_offset = range_last_offset

    return events, last_offset, errors


def _query_kedo_process_events_range(
    client: httpx.Client,
    settings: Settings,
    access_token: str,
    api_key: str,
    ...
    *,
    ...
    for _ in range(SIGNED_EVENTS_MAX_PAGES_PER_RANGE):
        page_query = dict(query)
        if from_time is not None and to_time is not None:
            page_query["eventTimeRange"] = {
                "from": _kedo_datetime(from_time),
                "to": _kedo_datetime(to_time),
            }
        if next_offset:
        ...
        try:
            ...
            if error.status_code != 0:
                raise
            if _can_split_event_range(from_time, to_time):
                assert from_time is not None
                assert to_time is not None
                midpoint = from_time + ((to_time - from_time) / 2)
                newer_events, newer_offset = _query_kedo_process_events_range(
                    client,
                    settings,
                    access_token,
                    api_key,
                ...
                older_events, older_offset = _query_kedo_process_events_range(
                    client,
                    settings,
                    access_token,
                    api_key,
                ...
                return newer_events + older_events, older_offset or newer_offset

            errors.append(_event_query_error(error, from_time, to_time))
        ...
        page_events = _as_object_list(payload.get("events"))
        events.extend(page_events)
        page_last_offset = _string_value(payload, "lastOffset")
        if page_last_offset:
            last_offset = page_last_offset
        if not page_last_offset or page_last_offset == next_offset or len(page_events) < page_limit:
            break
        next_offset = page_last_offset

    return events, last_offset


def _can_split_event_range(
    from_time: datetime | None,
    to_time: datetime | None,
) -> bool:
    if from_time is None or to_time is None:
        return False
    return to_time - from_time > SIGNED_EVENTS_MIN_RANGE


def _event_query_error(
    error: KedoApiError,
    from_time: datetime | None,
    to_time: datetime | None,
) -> dict[str, Any]:
    return {
        ...
        "statusCode": error.status_code,
        "message": error.response_text,
        "from": _kedo_datetime(from_time) if from_time is not None else None,
        "to": _kedo_datetime(to_time) if to_time is not None else None,
    }
        ...
        for document_key, signed_content in _signed_contents(node):
            ...
            signed_documents.append(
                KedoSignedDocument(
                    ...
                    raw_signed_content=signed_content,
                    raw_signature_wrapper=signature_wrapper,
...
def _kedo_event_time_ranges(
    ...
    while end > from_time:
        start = max(from_time, end - timedelta(days=1))
